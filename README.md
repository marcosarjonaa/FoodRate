...
- `onBindViewHolder`:  
  Asocia los datos de una receta específica (`listRecetas[position]`) a su respectivo `ViewHolder`.
...
### **Métodos:**
- `renderize`:  
  Muestra los datos de una receta (`Recetas`) en las vistas:
  - Usa `Glide` para cargar imágenes de manera eficiente.
  - Configura un listener para eliminar recetas (`setOnClickListener`).

- `setOnClickListener`:  
  Establece el comportamiento al pulsar el botón de eliminar (`ivDelete`).
...
- `layoutManager`: Define cómo se organiza el `RecyclerView` (en este caso, lineal).
...
4. El usuario puede eliminar recetas mediante `deleteRecetas`, que actualiza la lista y el adaptador.
